  ...
  const rowAction = (c: LibCard) => {
    if (confirmId === c.id) {
      return (
        <button
          ...
          className="text-xs border border-red-900 text-red-400 rounded-md px-2.5 py-1 hover:bg-red-950/60 transition-colors"
        >
          确认删除
        </button>
      );
    ...
    return (
      <button
        ...
        className="text-xs border border-[var(--border)] t3 rounded-md px-2.5 py-1 hover:border-[var(--accent)] accent-text transition-colors"
      >
        {c.suspended ? "恢复" : "搁置"}
      </button>
    );
    ...
    <div className="min-h-screen flex flex-col items-center px-6 pt-12 pb-10">
      ...
      <div className="mt-4 w-full max-w-xl flex flex-col gap-2 pb-10">
        ...
        {cards.map((c) => {
          ...
          return (
            <div key={c.id} className="surface rounded-lg px-4 py-2.5 flex items-center gap-3">
